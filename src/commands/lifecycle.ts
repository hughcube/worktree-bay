...
export async function stopCommand(cfg: BayConfig, feature: string, service?: string) {
  await withLock(cfg.workspaceRoot, async () => {
    let any = false
    for (const o of occupantsOf(cfg, feature, service)) { if (!hasRuntime(cfg, o.service)) continue; any = true; log(c.bold(c.cyan(o.service))); await stopRuntime(ctxOf(cfg, o)) }
...
export async function startCommand(cfg: BayConfig, feature: string, service?: string) {
  await withLock(cfg.workspaceRoot, async () => {
    let any = false
    for (const o of occupantsOf(cfg, feature, service)) { if (!hasRuntime(cfg, o.service)) continue; any = true; log(c.bold(c.cyan(o.service))); await ensureRuntime(ctxOf(cfg, o)) }
