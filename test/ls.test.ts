...
describe('ls', () => {
  it('列出 claim 槽 + 服务端口', () => { claim(cfg, 'feat-a'); fs.mkdirSync(path.join(ws, 'api', '.worktrees', 's1-feat-a'), { recursive: true }); const out = strip(renderSlots(cfg)); expect(out).toMatch(/^1: feat-a/m); expect(out).toContain('api@6002') })
  it('通用：非 api 服务不出现硬编码 "api="，显示真实服务名@端口，不再有 block=', () => {
    ...
    const out = strip(renderSlots(c2))
    expect(out).toMatch(/^1: f/m); expect(out).toContain('web@6002'); expect(out).not.toContain('block='); expect(out).not.toContain('api=')
